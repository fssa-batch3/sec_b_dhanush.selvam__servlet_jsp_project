		...
		editBtn.forEach((e) => {
			e.addEventListener("click", function () {
				...
				if (idField.value !== id){
					...
					initialLeaveType = leave.leaveType; 
				}
				...
				nameField.addEventListener("blur", function () {
		            if (initialLeaveType !== nameField.value && isEditClicked) {
		                const isDuplicate = duplicateLeavesList.some((leave) => leave.leaveType === nameField.value);
		                if (isDuplicate) {
		...
		document.querySelector(".submit_btn").addEventListener("click",(e) => {
		    ...
		    nameField.addEventListener("blur", function () {
	            if (initialLeaveType !== nameField.value && isEditClicked) {
	                const isDuplicate = duplicateLeavesList.some((leave) => leave.leaveType === nameField.value);
	                if (isDuplicate) {
	                    alert("Leave name already exists.");
	                    nameField.value = "";
	                    nameField.focus();
	                    return;
	                }
